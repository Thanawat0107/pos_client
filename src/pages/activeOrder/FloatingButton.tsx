...
export default function FloatingButton({ activeOrders, onClick }: Props) {
  ...
  // 2. ตรวจสอบว่าออเดอร์ล่าสุดอยู่ในสถานะที่ควรโชว์เลขคิวหรือไม่
  const showQueue = !HIDDEN_QUEUE_STATUSES.includes(latestOrder.orderStatus);

  const statusConfig = getStatusConfig(latestOrder.orderStatus);
  const isAnyReady = activeOrders.some(
                ...
                <Box
                  sx={{
                    ...
                    background: isMultiple
                      ? `conic-gradient(${activeOrders.map((o, i) => `${getStatusConfig(o.orderStatus).iconColor} ${(i / activeOrders.length) * 360}deg ${((i + 1) / activeOrders.length) * 360}deg`).join(", ")})`
                      : "transparent",
              ...
              <>
                ...
                >
                  {isMultiple ? (
                    ...
                    <Box sx={{ color: "#fff" }}>
                      {statusConfig.icon || <ReceiptLongIcon />}
                    </Box>
                  )}
                </Box>
                  ...
                  >
                    {
                      isMultiple
                        ? "ติดตามสถานะออเดอร์"
                        : showQueue && latestOrder.pickUpCode
                          ? `คิวรับอาหาร: ${latestOrder.pickUpCode}`
                          : "กำลังตรวจสอบออเดอร์..."
                    }
                  </Typography>
                        ...
                        {activeOrders.map((o) => (
                          <Box
                            ...
                            sx={{
                              ...
                              height: 10,
                              borderRadius: "50%",
                              bgcolor: getStatusConfig(o.orderStatus).iconColor,
